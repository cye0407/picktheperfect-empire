...
export function runMatching({
  ...
  scoreItem: (item: Coffee, prefs: CoffeePreferences) => { score: number; maxScore: number } | null;
  getReasons: (item: Coffee, prefs: CoffeePreferences) => string[];
}) {
  const scoredItems = items
    ...
    .filter((item): item is (Coffee & { matchScore: number; matchPercentage: number; matchTier: string; matchReasons: string[] }) => item !== null)
